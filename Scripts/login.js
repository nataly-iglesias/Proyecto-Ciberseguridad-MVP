document.querySelector('form').addEventListener('submit', async function(e) {
    ...
    if (bloqueoHasta && ahora < bloqueoHasta) {
        ...
        alert(`Has alcanzado el número máximo de intentos. Espera ${segundosRestantes} segundos para intentarlo de nuevo.`);
        return;
    }
    ...
    try {
        ...
        if (response.ok && data.token) {
            ...
            if (nuevosIntentos >= 3) {
                ...
                alert('Demasiados intentos. Espera 30 segundos para volver a intentarlo.');
            }
        }
    } catch (error) {
